...
网络就是一堆设备，连接到一起（物理连接或虚拟连接），然后相互交换信息。
...
唯一的一层，和tcp/ip没有什么关系。有些硬件不仅仅只实现物理层的功能，如以太网的网卡就实现了链路层的功能。

# 链路层 ISO中为L2

为方便，处理硬件和介质的问题都放到同一层，这就是链路层。如各种网卡驱动。链路层只能处理直接相连的主机的数据传输问题。不同的网络，链路层的实现可能不同。
...
两台设备之间的连接通信，是对SLIP的替换。包含一系列子协议。
